...
* a variable that is important in most of the models Skibiel et al 2013 fit is "relative lactation duration." or "relative lacation length" 
...
* This transformation to a proportion corrects for the fact that 
    ...
    + and hence a longer periods of lacation as these large babies mature.
* We'll look indepth at their models for % fat~lactation duration to understand model selection and model comparison.
...
* (recall that log = natural lon in R)
...
* An untransformed x axis might be better
...
* "rel.lactation" is a proportion so perhpas the logit would be betetr
...
Fit models of untransformed lactation duration and compare to log and logit transformations
...
Resid vs. fitted looks pretty bad - definate triangle shape.
```{r}
par(mfrow = c(2,2))
plot(f.rellact.raw)
```
...
* the problem may be worse using log transformed relative laction duration as a predictor (that is, a long transformed x axis).
...
* They used relative lactation duration as a continous predictor of the fat content of milk, & also several categorical variables: 
...
* Also becase they are defined with a "+" sign
...
* AIC for the log(fat) model is MUCH MUCH lower that logit(fat) model we jsut fit
...
* What might be the impact of having missing data (NAs) for some of your predictors but not others?  
...
The original and "fake" modles with just lacations duration as a predictor are the same b/c they are fit to the same dataset
```{r}
AICtab(base = TRUE,
       m.rel.lac,
       fake.m.rel.lac)
...
The original and "fake" modles with "diet" add have different AICs because they are fit to differnet datas
```{r}
AICtab(base = TRUE,
       m.rel.lac.diet,
       fake.m.rel.lac.diet)
...
* To make our life simpler we'll just consider the model that had lactation duration and diet 
    + That is, we'll ingore the "biome" predictor.  
...
* Note use of indicies to select the subset of data being used
    + data = dat2[i.carn,] for canivores
    + data = dat2[i.herb,] for herbivores
* Legend gets added with length()
    + ggplot also makes pretty good legend on the fly
    + why don't I just invest the effort to each ggplot from the begining...
...
* This uses the regression equation from the specificed model (f.lact.biome.diet) and plugs in observed values (xs) for the 3 predictors (relative lactation duration, biome, and diet).
...
The range of lacation duration values
```{r}
...
Create a new dataframe from which to generate predictiosn

* This uses the expand.grid function
* This generates all combinations of the varibles that are given
...
* use predict()
* give it themodel in question, "f.lact.diet""
...
Add the y.hat values (predictions) to the newdat datafrme
...
**QUESTION**
It looks like the herbiore and omnivore lines are almost exaclty on top of each other. What does this mean?
...
The "diet" catagory has 3 levels
...
* -0.58 is the differene between carnviores and herbivores
* -0.57 is the differene between carnviores and omnivore


# Back to heterogeneity of variance

* Our regression line for carnivorees fits those species pretty well, but the omni- and herbivore lines don't fit their data well.  
* This indicates the source of our hetergenity of variance problem:
...
Here's a trick: I"m going to put all of our plotting code intoa function

---
title: 'Duq Biostats Week 6 | Regression Tutorial 7: Interactions'
...
* Note the triangle shape of the data.  
    * This forebodes heterogenous variance.
...
* In the model selection tutorial we saw that herbivores and omnivores have almost idential slopes.  
...
Look at the column with sumamry()
```{r}
summary(diet2)
```
...
* The ifelse() command coverted the data from a factor to character data.  
...
* 1 Intercept
* Slope based on continous predictor, rel.lactation
...
* 1 Intercept for carnivorwes
* 1 intercept for herbivores+omni
* No slope; models 2 falt lines
...
* (This term - analysis of covariance - is only really useful is you were taught expliclity about ANCOVA)
...
* Models with an interation term have lines with slopes that are not parralle
...
* R expands out the terms invovled in the interaction
...
* m.lact.X.diet vs m.lact.diet (interaction vs. additive) had dAIC of 0.3 and p = 0.12.  THe high -- but not really really high -- matches with the conclusion that the itneraction is not well supported, but not obviously bad
...
* THe qq normal plot tells you about the normality of the residul.  On the left-hand size its look a little bad; It would be interesting to know if the logit transformation improves this at all.
...
Plot scatterplot w/ modle and label the points with large resiudals
...
Plot scatter plot with regression lines vs. residvs. fitted plot
```{r}
...
* R2 reprents the amount of variation that a model explains
* Its a ratio of the sum of squares of the model in equation over the sum of squares of null model
* R2 = SS/RSS.null
* "SS" is the sume of square from the focal model
...
### Gettig this information from using just anova(model)
